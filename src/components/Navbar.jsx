...
const Navbar = () => {
   ...
   <>
    <nav className='bg-white md:px-14 p-4 max-w-screen-2xl  border-b mx-auto text-primary fixed top-0 right-0 left-0'>
        <div className='text-lg container mx-suto flex justify-between items-center font-meduim'>
            <div className='flex space-x-14 items-center'>
<a href ="/" className='text-2xl font-semibold flex items-center space-x-3 text-primary'>
    <img src={logo} alt="" className='w-10 inline-blocks items-center'/><span>XYZ</span>
    </a>
    ...
    <div className='space-x-12 hidden md:flex items-center'>   
        ...
        <button className='bg-secondary py-2 px-4 transition-all duration-300 rounded hover:bg-white hover:text-primary'>Sign up</button>
    </div>
...
<button onClick={toggleMenu} className='text-white focus:outline-none focus:text-gray-300'>
{
    ...
    <div className={`space-y-4 px-4 pt-24 pb-5 bg-secondary text-xl ${isMenuOpen ? " block fixed top-0 right-0 left-0" : " hidden "}`}>
        {
            navItems.map(({link,path}) => <Link activeClass='active' spy={true} smooth={true} offset={-80}
             key={link} to={path} className='block text-white hover:text-gray-300'
             onClick={toggleMenu}
             >{link}</Link>)
        }
